...
do while .t.
   ...
   do while .t. // Loop jogada
      ...
      cSubString := SubStr(cStringLetrasJogo, 2, 19)
      cStringLetrasJogo := cSubString + cLetra
   ...
   do while nContador <= nJogadores
      cJogadorTabela := SubStr(cJogadores, (20 * nJogadores) - 19, 20)
      cPontuacaoTabela := SubStr(cPontuacoes, (3 * nJogadores) - 2, 3)
      Alert(cPontuacaoTabela)
      @ nLinha,01 say AllTrim(cJogadorTabela)
      @ nLinha,33 say AllTrim(cPontuacaoTabela)
      if nLinha >= 10
         nLinha := 3
         @ 03,01 clear to 25,80
      else
         nLinha++
      endif
      nContador++
